    ...
    T: Display + Send + Sync + 'static,
{
    ...
    let h1 = tokio::spawn(accept_and_reply(first, reply.clone()));
    let h2 = tokio::spawn(accept_and_reply(second, reply));

    tokio::join!(h1, h2);
}

pub async fn accept_and_reply<T>(listener: TcpListener, reply: Arc<T>)
where
    T: Display,
{
    loop {
